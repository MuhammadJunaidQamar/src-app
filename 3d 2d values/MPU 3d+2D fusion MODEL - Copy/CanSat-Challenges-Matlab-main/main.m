...
xlabel(ax2D,'X Position (m)');
ylabel(ax2D,'Y Position (m)');
title(ax2D,'2D Path + Distance');
xlim(ax2D,[-200 200]);
ylim(ax2D,[-200 200]);
...
distText = text(ax2D, 0.05, 0.95, sprintf('Distance: %.2f m', totalDist), ...
    'Units','normalized','FontSize',12,'FontWeight','bold','Color',[0.1 0.1 0.5]);
...
uicontrol('Style','text','String','2D Zoom','Position',[1050 460 120 20],'BackgroundColor',[0.95 0.9 0.85]);
zoomSlider = uicontrol('Style','slider','Min',0.5,'Max',5,'Value',1,'Position',[1050 440 120 20]);

uicontrol('Style','text','String','Speed Sensitivity','Position',[1050 400 120 20],'BackgroundColor',[0.95 0.9 0.85]);
speedSlider = uicontrol('Style','slider','Min',0.1,'Max',5,'Value',1.0,'Position',[1050 380 120 20]);

resetBtn = uicontrol('Style','pushbutton','String','Reset 2D View','Position',[1050 340 120 30], ...
    'BackgroundColor',[0.8 0.9 1],'Callback',@(~,~)resetView());
clearBtn = uicontrol('Style','pushbutton','String','Clear 2D Path','Position',[1050 300 120 30], ...
    'BackgroundColor',[1 0.8 0.8],'Callback',@(~,~)clearPath());

    function resetView()
        xlim(ax2D,[-200 200]); ylim(ax2D,[-200 200]);
    end
    function clearPath()
        pathX = 0; pathY = 0; x = 0; y = 0; totalDist = 0;
        set(hPath,'XData',pathX,'YData',pathY);
        set(hPoint,'XData',x,'YData',y);
        set(distText,'String',sprintf('Distance: %.2f m', totalDist));
        disp("🧹 Path cleared.");
    end

%% === Variables ===
dataReceived = false;
prevRoll = 0; prevPitch = 0; prevYaw = 0;
...
while isvalid(s) && isvalid(fig)
    try
        ...
        if ~isempty(tokens)
            ...
            %% === Motion Filtering ===
            % Deadzone threshold for small movements (in degrees)
            tiltThreshold = 2;  
            moveRoll = abs(roll - prevRoll) > tiltThreshold;
            movePitch = abs(pitch - prevPitch) > tiltThreshold;
            moveYaw = abs(yaw - prevYaw) > tiltThreshold;

            % Only update movement if CanSat is actually tilting significantly
            if moveRoll || movePitch || moveYaw
                sensitivity = 0.1; % m per degree of tilt
                spd = speedSlider.Value;

                % Apply smoothing (reduce noise)
                roll = 0.8*prevRoll + 0.2*roll;
                pitch = 0.8*prevPitch + 0.2*pitch;

                dx = spd * sensitivity * sin(deg2rad(pitch));
                dy = spd * sensitivity * sin(deg2rad(roll));

# main_3d.py - Enhanced Main with 3D Visualization + WebSocket
import asyncio
import csv
import json
import logging
...
from main_gui_3d import MissionGUI3D
import sys
...
def main():
    ...
    if websocket_url:
        try:
            ...
            print(f"⚠️ WebSocket publishing disabled ({exc})")
            ws_publisher = None
    else:
        print("ℹ️ WebSocket publishing disabled. Set CANSAT_WS_URL to enable streaming.")

    # --- Tkinter root GUI setup for CanSat 3D ---
    root = tk.Tk()
    root.title("🛰️ CanSat Mission Control - 3D Enhanced")
    root.configure(bg="#070814")
    root.geometry("1400x900")
    
    gui = MissionGUI3D(root)
    ...
    def cansat_packet_callback(data):
        ...
        try:
            data["PythonTimestamp"] = gui.environment.getPythonTimestamp(data)
            
            # Update GUI (schedule on Tkinter thread)
            ...
            if now - cansat_packet_callback.last_update >= 0.05:  # ~20 Hz update
                ...
                try:
                    root.after(0, lambda d=data.copy(): gui.on_new_packet(d))
                except tk.TclError:
                    # Window was closed
                    pass
    ...
    if ws_publisher:
        print(f"🌐 Streaming to: {websocket_url}")
    print("="*60)
